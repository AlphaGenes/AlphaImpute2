...
def impute_individuals_on_chip(ld_individuals, args, haplotype_library):
    ...
    forward_loci, reverse_loci = get_non_missing_loci(ld_individuals, args.phasing_loci_inclusion_threshold)
    ...
    multi_threaded_apply(backward_impute_individual, [ind.reverse_individual() for ind in ld_individuals], reverse_library, args.n_imputation_particles, args.length*args.imputation_length_modifier)
    ...
    multi_threaded_apply(forward_impute_individual, ld_individuals, forward_library, args.n_imputation_particles, args.length*args.imputation_length_modifier)
...
def backward_impute_individual(reverse_individual, library, n_samples, map_length):
    ...
    ParticlePhasing.phase(reverse_individual, library, set_haplotypes = False, imputation = True, n_samples = n_samples, map_length = map_length)
...
def forward_impute_individual(individual, library, n_samples, map_length):
    ...
    ParticlePhasing.phase(individual, library, set_haplotypes = False, imputation = True, n_samples = n_samples, map_length = map_length)
